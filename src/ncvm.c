...
_export void ncvm_free_thread(ncvm_thread* thread) {
...
#define ADDR_REGISTER result.u64_registers[0]
#define EXECUTE_COMMAND(IP)\
    switch (IP->opcode) {\
        case STOP:\
            /* TODO: add regisetr */\
            goto while_exit;\
...
_export  u8 ncvm_execute_thread_step(ncvm_thread* thread) {
    ...
    register const Instruction* IP = si_p;
    while (true) {
        switch (IP->opcode) {
            case STOP:
}
